...
import edu.grinnell.sortingvisualizer.sortevents.DummyEvent;
...
public class ControlPanel extends JPanel {
    ...
    //60 fps F.T.W. (144hz if possible)
        ...
        playButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                List<SortEvent<Integer>> events = generateEvents((String) sorts.getSelectedItem(),
                        notes.getNotes().clone());

                int k = notes.getNotes().length;
                for (int i = 0; i < k-1; i++) {
                    CompareEvent<Integer> event = new CompareEvent<Integer>(i, i+1);
                    events.add(event);
                }
                //adding a dummyEvent to clear the highlights for a final 
                DummyEvent<Integer> event2 = new DummyEvent<Integer>();
                events.add(event2);
                    ...
                    public void run() {
                        if (index < events.size()) {
                            ...
                            for (int i = 0; i < e.getAffectedIndices().size(); i++) {
                                scale.playNote(notes.getNotes()[e.getAffectedIndices().get(i)],
                                        e.isEmphasized());
